...
namespace FIMSpace.AnimationTools
{
    ...
    {
        public Transform LatestAnimator { get; internal set; }
        ...
        internal void CopySettingsFromTo(AnimationClip fromClip, int from, int to, AnimationDesignerSave toSave, AnimationClip toClip)
        {
            ...
            for( int i = 0; i < toSave.MainSetupsForClips.Count; i++ )
                if( toSave.MainSetupsForClips[i].SetIDHash == to && MainSetupsForClips[i].SettingsForClip == toClip ) { toSave.MainSetupsForClips[i].ApplySettingsOf( myMain ); break; }
            ...
            ADClipSettings_CustomModules myModules = GetSetupForClip(CustomModuleSetupsForClips, fromClip, from);
            for (int i = 0; i < toSave.CustomModuleSetupsForClips.Count; i++)
                if (toSave.CustomModuleSetupsForClips[i].SetIDHash == to && CustomModuleSetupsForClips[i].SettingsForClip == toClip) { toSave.CustomModuleSetupsForClips[i] = myModules.Copy(CustomModuleSetupsForClips[i], toSave, true); break; }
