...
namespace FIMSpace.AnimationTools
{
    public partial class AnimationDesignerWindow : EditorWindow
    {
        ...
        public void RefreshClipLengthModValues()
        {
            ...
            if (currentMecanim != null) if (currentMecanim.isHuman) if (Anim_MainSet != null)
            ...
            if (isHumanoidIKSetup && currentClipSettings != null)
            {
                ...
                if (latestSecondaryAnimator != null)
                    if (latestSecondaryAnimatorClip != null)
                    {
                        if (latestSecondaryAnimatorClip.length != 0f && TargetClip.length != 0f)
                        {
                            ...
                            {
                                Quaternion secAnimRotation = Quaternion.Euler(seconadaryAnimatorRotation);

                                Animator secAnim = latestSecondaryAnimator.gameObject.GetComponent<Animator>();

                                if (secAnim && secAnim.applyRootMotion && latestSecondaryAnimatorClip.hasMotionCurves) // For root motion additive
                                {
                                    latestSecondaryAnimator.transform.position = seconadaryAnimatorPosition + secAnimRotation * latestSecondaryAnimator.position;
                                    latestSecondaryAnimator.transform.rotation *= secAnimRotation;
                                }
                                else
                                {
                                    latestSecondaryAnimator.transform.position = seconadaryAnimatorPosition;
                                    latestSecondaryAnimator.transform.rotation = secAnimRotation;
                                }
                            }
                        }
        ...
        void StartBake(AnimationClip originalClip, AnimationDesignerSave save, ADArmatureSetup armature)
        {
            armature.Bake_Prepare(latestAnimator, originalClip, save, _anim_MainSet);
        }


        void BakingLoop(ref AnimationClip clip, int i, int keys, int startKey, int endKey, ADArmatureSetup armature)
        {
            if (bakeFramerateRatio <= 1.05f || i == startKey || i == endKey) // No need for oversimulating
            {
                ...
                UpdateSimulationAfterAnimators(armature.rootBake);

                LateUpdateSimulation();

                armature.Bake_CaptureFramePose(animationElapsed);
            }
        ...
        void FinishBake(ref AnimationClip clip, AnimationClip originalClip, ADClipSettings_Main main, ADArmatureSetup armature, AnimationDesignerSave save)
        {
            armature.Bake_Complete(ref clip, save, originalClip, main);
        }
