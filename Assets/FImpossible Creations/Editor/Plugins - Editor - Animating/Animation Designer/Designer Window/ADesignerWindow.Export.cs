...
namespace FIMSpace.AnimationTools
{
    public partial class AnimationDesignerWindow : EditorWindow
    {
        ...
        public void ExportToFile(string path, bool rememberSavePath = false, bool forceAllCurves = false)
        {
            isBaking = true;
            ...
            try
            {
                var targetArmature = Ar;
                var targetSetup = S;

                if ( forceAllCurves) // Generate export-only temporary setup and armature
                {
                    targetSetup = GameObject.Instantiate(targetSetup);
                    targetSetup.LatestAnimator = S.LatestAnimator;

                    targetArmature = targetSetup.Armature;
                    targetArmature.RootBoneReference = Ar.RootBoneReference;
                    targetArmature.PelvisBoneReference = Ar.PelvisBoneReference;
                    targetArmature.LatestAnimator = Ar.LatestAnimator;

                    for (int b = 0; b < targetArmature.BonesSetup.Count; b++)
                    {
                        targetArmature.BonesSetup[b].TempTransform = Ar.BonesSetup[b].TempTransform;
                        targetArmature.BonesSetup[b].CompressionFactor = Ar.BonesSetup[b].CompressionFactor;
                    }

                    targetArmature.VerifyArmatureWithAnimationClip(TargetClip, false, targetSetup);

                    foreach (var limb in targetArmature.BonesSetup)
                    {
                        limb.BakeBone = true;
                        limb.BakePosition = true;
                        limb.BakeScale = true;
                        //UnityEngine.Debug.Log("Limb: " + limb.BoneName + " - " + limb.TempTransform);
                    }
                }

                _anim_MainSet = targetSetup.GetSetupForClip(targetSetup.MainSetupsForClips, TargetClip, _toSet_SetSwitchToHash);
                ...
                StartBake(originalClip, targetSetup, targetArmature);
                ...
                for (int i = startKey; i <= endKey; i++)
                {
                    ...
                    BakingLoop(ref newGeneratedClip, i, keys, startKey, endKey, targetArmature);
                ...
                FinishBake(ref newGeneratedClip, originalClip, _anim_MainSet, targetArmature, targetSetup);
